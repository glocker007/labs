...
struct Statistics{
  virtual double ChangeValue(const Point& point,const shared_ptr<Potencial::Potencial> potencial) const = 0;
...
InternalEnergyPerParticle();
  ...
  double Eint;
  size_t N;
  double ChangeValue(const Point& point,const shared_ptr<Potencial::Potencial> potencial){
    Eint+=s( 
  }
};
...
Temperature():T(0){}
  ...
  double ChangeValue(const Point& point,const shared_ptr<Potencial::Potencial> potencial) {
    T+=s(point.velocity,point.velocity)*point.mass/(3*N); 
    return T;
  }
};
